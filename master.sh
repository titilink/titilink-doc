...
bootstrap_daemon() {
    ...
    ${docker_daemon} \
        ...
        --ip-masq=false \
        --bridge=none \
        --dns 10.1.7.88 --dns 10.1.7.77 --insecure-registry=docker.hikvision.com.cn --insecure-registry=10.33.40.144 \
...
start_k8s(){
    ...
    docker -H unix:///var/run/docker-bootstrap.sock run \
        ...
        --net=host \
        -d \
        10.33.40.144/google_containers/etcd-${ARCH}:${ETCD_VERSION} \
        /usr/local/bin/etcd \
    ...
    docker -H unix:///var/run/docker-bootstrap.sock run \
        --net=host 10.33.40.144/google_containers/etcd:${ETCD_VERSION} \
        etcdctl \
    ...
    flannelCID=$(docker -H unix:///var/run/docker-bootstrap.sock run \
        ...
        --privileged \
        -v /dev/net:/dev/net \
        10.33.40.144/flannel:${FLANNEL_VERSION} \
        /opt/bin/flanneld \
    ...
    docker run \
        ...
        10.33.40.144/google_containers/hyperkube-${ARCH}:v${K8S_VERSION} \
        /hyperkube kubelet \
            --address=0.0.0.0 \
